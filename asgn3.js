...
const FSHADER_SOURCE = `
  ...
  //===== ADDED: sampler for blue yarn
  uniform sampler2D u_SamplerBlueYarn;
  ...
  void main(){
    ...
    //===== ADDED: if textureNum == 4 => blue yarn
    else if(u_whichTexture == 4){
      texColor = texture2D(u_SamplerBlueYarn, v_UV);
    } 
    else if(u_whichTexture == -2){
      // color only
...
function main(){
  ...
  u_SamplerBlueYarn  = gl.getUniformLocation(gl.program,'u_SamplerBlueYarn');
...
function initWorldData(){
  ...
  for(let i=0; i<WORLD_SIZE; i++){
    g_mapData[i][0] = 3; 
    g_mapData[i][WORLD_SIZE-1]=3;
    g_mapTexture[i][0] = 3; 
    g_mapTexture[i][WORLD_SIZE-1] = 3;
    g_mapData[0][i] = 3;
    g_mapData[WORLD_SIZE-1][i] = 3;
    g_mapTexture[0][i] = 3;
...
// yarn tower (2D columns, using brick texture)
...
function spawnYarnRocket(){
  ...
  for(let px=0; px<2; px++){
    for(let pz=0; pz<2; pz++){
      g_blocks[baseX+1+px][baseZ+1+pz][1] = 4;
    }
  }
  // top block
  g_blocks[baseX+1][baseZ+1][2] = 4;

  // corner fins
  g_blocks[baseX][baseZ][1] = 4;      
  g_blocks[baseX+2][baseZ][1] = 4;    
  g_blocks[baseX][baseZ+2][1] = 4;
  g_blocks[baseX+2][baseZ+2][1] = 4;
...
function placeBlock3D(){
  ...
  let forward = new Vector3(g_camera.at.elements);
  forward.sub(g_camera.eye);
  forward.normalize();

  // Move about 1 unit in front
  let px = g_camera.eye.elements[0] + forward.elements[0];
  let py = g_camera.eye.elements[1] + forward.elements[1];
  let pz = g_camera.eye.elements[2] + forward.elements[2];

  let x = Math.floor(px + WORLD_WIDTH/2);
  let y = Math.floor(py);
  let z = Math.floor(pz + WORLD_DEPTH/2);

  if(x>=0 && x<WORLD_WIDTH && y>=0 && y<WORLD_HEIGHT && z>=0 && z<WORLD_DEPTH){
    g_blocks[x][z][y] = 3; 
    console.log("3D block (wood) placed at x=",x,"y=",y,"z=",z);
  }
}
...
function initTextures(){
  ...
  blueYarnImg.onload = ()=>{ sendTexToGPU(blueYarnImg, blueYarnTex, 4, u_SamplerBlueYarn); };
